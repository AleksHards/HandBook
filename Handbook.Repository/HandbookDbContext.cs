...
public class HandbookDbContext : DbContext
{
    ...
    protected override void OnModelCreating(ModelBuilder modelBuilder)
    {
        ...
        modelBuilder.Entity<Person>().HasMany(pl => pl.PersonLink).WithOne(p => p.Person).IsRequired(false);
        modelBuilder.Entity<Person>().HasMany(pl => pl.PersonLink).WithOne(p => p.Relative).IsRequired(false);
        ...
        modelBuilder.Entity<PersonLink>().HasOne(p => p.Person).WithMany(pl => pl.PersonLink).IsRequired(false);
        modelBuilder.Entity<PersonLink>().HasOne(p => p.Relative).WithMany(pl => pl.PersonLink).IsRequired(false);
    }
